...
<head>
    ...
    <title>Chiffrement et déchiffrement de DES</title>
...
class DataEncryptionStandard {
    ...
    private $log = array();
    private $k   = array();
    // la premiere etape : Permutation initiale
    ...
    // la derniere etape : Permutation finale
    ...
    // Fonction de processus DES
    private function encryptBlock($message){
    ...
    $blockSize = 8;
    $numBlocks = ceil(strlen($message) / $blockSize);
    $result = '';

    for ($blockIndex = 0; $blockIndex < $numBlocks; $blockIndex++) {
        // Extraire un bloc de 8 caractères
        $block = substr($message, $blockIndex * $blockSize, $blockSize);

        // Chiffrer le bloc
        $encryptedBlock = $this->encryptBlock($block);

        // Ajouter le résultat au résultat global
        $result .= $encryptedBlock;
    }

    return $result;
}
    // Appliquons le Chiffrement de DES
    public function chiffrer($message, $key){
        // Vérifier si la longueur n'est pas un multiple de 8
        ...
        // Step 1: IP permutation
        $binCipherText = $this->hexToBin($cipherText);
        $sourceIP = $this->permutation($this->IP, $binCipherText);

        // Step 2: IP splitting
        $L = array();
        $R = array();
        $L[16] = substr($sourceIP, 0, 32);
        $R[16] = substr($sourceIP, 32, 32);

        // Step 3: Iterations (in reverse order)
        for ($i = 15; $i >= 0; $i--) {
            // set Rn
            $R[$i] = $L[$i + 1];

            // Step 3.1: E permutation
            $EL = $this->permutation($this->E, $R[$i]);

            // Step 3.2: XOR with subkey
            $EL = $this->dataXOR($EL, $this->k[$i + 1]);

            // Step 3.3: S-box transformation
            $s = $this->sbox($EL);

            // Step 3.4: P permutation
            $f = $this->permutation($this->P, $s);

            // set Ln
            $L[$i] = $this->dataXOR($R[$i + 1], $f);
        }
        ...
        // Step 5: IP-1 permutation
        $encoded = $this->permutation($this->IP1, $concat);

        // Convert result into hex
        $result = '';
        for ($start = 0; $start < strlen($encoded); $start += 4) {
            $result .= strtoupper(base_convert(substr($encoded, $start, 4), 2, 16));
        }

        return $result;
    }

    public function dechiffrer($cipherText, $key) {
    ...
    private function decryptMessage($cipherText) {
        $blockSize = 16; // Twice the block size for hex encoding
        $numBlocks = ceil(strlen($cipherText) / $blockSize);
        $result = '';

        for ($blockIndex = 0; $blockIndex < $numBlocks; $blockIndex++) {
            // Extract a block of 16 hex characters
            $block = substr($cipherText, $blockIndex * $blockSize, $blockSize);

            // Decrypt the block
            $decryptedBlock = $this->decryptBlock($block);

            // Add the result to the global result
...
?>
    <div class="form-group">
     ...
     <div class="container">
        <h1>Chiffrement de DES</h1>
        <div class="container">
        <form id="myForm" method="post">
     <div class="form-group">
                <label for="cle">La clé :</label>
                <input type="text" value="<?php if (isset($_POST['cle'])) { echo $_POST['cle']; } ?>" name="cle" id="cle" minlength = '8' maxlength = '8'placeholder="Entrez la clé" required />
 ...
 <textarea id="number" name="resultat" readonly><?php if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['chiffrer'])) {
        ...
        $resultat = $des->chiffrer($texte, $cle);
        if($resultat == false){
            $erreur = 'Attention : La longueur de texte clair/chiffré doit etre un multiple de 8 !';
        }else{
            echo $resultat;
        }
        ...
        $des = new DataEncryptionStandard();
    $resultat2 = $des->dechiffrer($texte, $cle);
    if ($resultat2 == false) {
        $erreur2 = 'Attention : La longueur de texte clair/chiffré doit être un multiple de 8 !';
    } else {
        echo $resultat2;
    }
    }
